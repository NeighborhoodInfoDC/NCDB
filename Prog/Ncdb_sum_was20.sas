/**************************************************************************
 ...
 summary files for Washington metro area (2020).
...
%macro Summary( year=, data=, geosuf=, geo=, geofmt= );
  ...
  ** Tract-level summary data set for &year **;
  ...
  data Ncdb_sum_&year._was20_tr00;
  ...
  */
  ...
  proc datasets library=work memtype=(data) nolist;
    modify Ncdb_sum_&year._was20_tr00;
  ...
  ** Convert to 2010 tracts **;
  ...
  %Transform_geo_data(
	  dat_ds_name=Ncdb_sum_&year._was20_tr00,
	  dat_org_geo=geo2000,
	  ...
	  wgt_prop_var=popwt,
	  wgt_wgt_var=popwt,
	  out_ds_name=Ncdb_sum_&year._was20_tr10,
  ...
  ** County-level data set for &year **;
  ...
  proc summary data=Ncdb_sum_&year._was20_tr00;
    where put( county, $ctym20f. ) ~= "";
    by county;
    var &Count_vars &Prop_vars;
    output out=Ncdb_sum_&year._was20_regcnt (drop=_type_ _freq_)
      sum(&Count_vars)= 
      mean(&Prop_vars)=;
  run;
...
%Summary( year=1990, data=Ncdb_1990_2000_was20, geosuf=tr00, geo=Geo2000, geofmt=$geo00a. )
%Summary( year=2000, data=Ncdb_lf_2000_was20, geosuf=tr00, geo=Geo2000, geofmt=$geo00a. )
...
%macro Combine( geo=, years=, revisions=%str(New file.) );
  ...
  ** Combine all for years for %upcase(&geo) **;
  ...
  %Finalize_data_set( 
    ...
    data=Ncdb_sum_was20_&geosuf,
    out=Ncdb_sum_was20_&geosuf,
    outlib=Ncdb,
    label="NCDB summary, Washington region (2020), &geolbl",
    sortby=&geo,
    /** Metadata parameters **/
    restrictions=None,
...
%Combine( geo=Geo2000, years=1990 2000, revisions=%str(New file.) )
%Combine( geo=Geo2010, years=1990 2000, revisions=%str(New file.) )
%Combine( geo=County, years=1990 2000, revisions=%str(New file.) )
