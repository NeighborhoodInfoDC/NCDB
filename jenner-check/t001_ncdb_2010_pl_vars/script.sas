/**************************************************************************
 ...
 Source macro:  Macros/NCDB_2010_pl_vars.sas  (%NCDB_2010_pl_vars)
 What it does:  The repo's autocall macro that derives NCDB 2010
                demographic variables (bridged-race population counts,
                single-race-alone and race-in-combination counts,
                race/ethnicity proportions, age and housing totals)
                from Census 2010 PL94-171 redistricting fields.

 This bundle keeps the macro definition byte-for-byte as written in the
 repo and exercises it against a small block of synthetic PL94-171 input
 (the NCDB source data are not part of this repo), then prints the
 computed NCDB variables so the result can be read at a glance.
**************************************************************************/

/* Synthetic PL94-171 input: three census blocks carrying every P1/P2/P3/H1 */
/* cell the macro reads. Values are illustrative, not real census counts.    */

data Census_pl_2010_dc;

  length GeoBlk2010 $ 15;

  /* Establish every PL94-171 field the macro reads, default zero */
  array p1 P0010001-P0010071;
  array p2 P0020001-P0020073;
  array p3 P0030001-P0030001;
  array h1 H0010001-H0010003;
  do over p1; p1 = 0; end;
  do over p2; p2 = 0; end;
  do over p3; p3 = 0; end;
  do over h1; h1 = 0; end;
  ...
  /* Block 1 */
  GeoBlk2010 = "110010001001000";
  P0010001 = 420;  P0010003 = 180; P0010004 = 150; P0010005 = 5;  P0010006 = 40; P0010007 = 20; P0010008 = 15; P0010009 = 10;
  P0020002 = 60;   P0020003 = 360; P0020005 = 150; P0020006 = 135; P0020007 = 4; P0020008 = 35; P0020009 = 12; P0020010 = 10; P0020011 = 14;
  P0030001 = 330;  H0010001 = 210; H0010002 = 190; H0010003 = 20;
  output;
  ...
  do over p3; p3 = 0; end;
  do over h1; h1 = 0; end;
  GeoBlk2010 = "110010002001005";
  P0010001 = 610;  P0010003 = 300; P0010004 = 210; P0010005 = 8; P0010006 = 55; P0010007 = 25; P0010008 = 7; P0010009 = 5;
  P0020002 = 90;   P0020003 = 520; P0020005 = 290; P0020006 = 190; P0020007 = 6; P0020008 = 50; P0020009 = 20; P0020010 = 6; P0020011 = 8;
  P0030001 = 470;  H0010001 = 300; H0010002 = 275; H0010003 = 25;
  output;

  /* Block 3: zero-population block to exercise the guarded proportion branch */
  do over p1; p1 = 0; end;
  do over p2; p2 = 0; end;
  do over p3; p3 = 0; end;
  do over h1; h1 = 0; end;
  GeoBlk2010 = "110010003002999";
  P0010001 = 0;
  H0010001 = 4; H0010002 = 0; H0010003 = 4;
  output;
...
** Apply the repo macro to the synthetic input **;

data Ncdb_2010_vars;

  set Census_pl_2010_dc;

  %NCDB_2010_pl_vars()

run;

** Show the key computed NCDB variables **;

proc print data=Ncdb_2010_vars noobs label;
  var GeoBlk2010 TRCTPOP1
      SHRWHT1N SHRBLK1N SHRHSP1N SHRNHW1N SHRNHB1N
      SHRWHT1 SHRBLK1 SHRHSP1
      ADULT1N CHILD1N TOTHSUN1 OCCHU1 VACHU1;
run;
